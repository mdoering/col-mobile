...
enum GBIFEndpoints {
    ...
    /// All entries are polygon styles — they pair with the `bin=hex&hexPerTile=73`
    /// query that `mapTileURLTemplate` always appends.
    ...
    /// - `bin=hex&hexPerTile=50` renders polygon styles as hexagonal density bins.
    static func mapTileURLTemplate(taxonId: String, style: String = defaultMapTileStyle, resolution: String = "1x") -> String {
        "\(baseURL.absoluteString)/v2/map/occurrence/density/{z}/{x}/{y}@\(resolution).png?srs=EPSG:3857&checklistKey=\(colChecklistKey)&taxonKey=\(taxonId)&style=\(style)&bin=hex&hexPerTile=50"
    }
}
